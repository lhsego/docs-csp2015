...
# Now let's compute the range of the power so we can use the same axes for all the plots
powerLims <- range(d$Power.KW)
tempLims <- range(d$OAT.F)
save(powerLims, tempLims, file = "plottingLims.Rdata")

# Now we create a function to plot the power usage in all 5 buildings for a single day
power.by.time <- function(x) {
  ...
  for (i in as.character(2:5)) {
    with(x[x$building == i,], lines(dateTime, Power.KW, col = as.numeric(i) - 1, lwd = 2))
  }
  ...
  # Returning NULL is required by trelliscope when the plotting function is base R code
  # (as opposed plots generated by lattice or ggplot)
  return(NULL)
...
# that will be calculated (and visualizable) for each plot.  Keep in mind
# that each cognostic should consist of a scalar value.  The input to the cognostics
# function ('x') is a dataframe that contains one split, or one subset, of the data.  In this case
# it is a dataframe that contains all the data for a given date.
